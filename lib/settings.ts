...
export function setIntervalMinutes(minutes: number): number {
  if (!Number.isInteger(minutes) || minutes < MIN_INTERVAL || minutes > MAX_INTERVAL) {
    throw new Error(`intervalMinutes must be an integer between ${MIN_INTERVAL} and ${MAX_INTERVAL}`);
  }
  const db = getDb();
  db.insert(settings)
...
export function setRetentionDays(days: number): number {
  if (!Number.isInteger(days) || days < MIN_RETENTION_DAYS || days > MAX_RETENTION_DAYS) {
    throw new Error(`retentionDays must be an integer between ${MIN_RETENTION_DAYS} and ${MAX_RETENTION_DAYS}`);
  }
  const db = getDb();
  db.insert(settings)
