...
export function loadAuthConfig(): AuthConfig {
  ...
  if (issuer && clientId && clientSecret) {
    ...
    console.warn('[auth] OIDC partially configured — set all of FASTCOM_OIDC_ISSUER/CLIENT_ID/CLIENT_SECRET or none. OIDC disabled.');
  }
...
export function LoginForm({
  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-col gap-4 max-w-sm mx-auto mt-24">
      <h1 className="text-2xl font-semibold text-center">Sign in to Fastcom</h1>
      <input
...
Run: `docker build -t fastcom-auth-test .` (may take several minutes)
Expected: success.

Smoke test (note the extra env vars):
```
docker rm -f fastcom-auth 2>/dev/null || true
docker run -d --name fastcom-auth -p 13006:3000 \
  -e AUTH_SECRET=$(openssl rand -base64 32) \
  -e FASTCOM_ADMIN_EMAIL=admin@example.com \
  -e FASTCOM_ADMIN_PASSWORD=hunter2hunter2 \
  fastcom-auth-test
sleep 8
docker logs fastcom-auth 2>&1 | tail -5
...
docker rm -f fastcom-auth
docker rmi fastcom-auth-test
```
...
```
docker build -t fastcom-auth-final .
docker run -d --name fastcom-auth-final -p 13007:3000 \
  -e AUTH_SECRET=$(openssl rand -base64 32) \
  -e FASTCOM_ADMIN_EMAIL=admin@example.com \
  -e FASTCOM_ADMIN_PASSWORD=hunter2hunter2 \
  fastcom-auth-final
...
# Fetch CSRF token:
CSRF=$(curl -sS -c /tmp/fastcom-cookies.txt http://localhost:13007/api/auth/csrf | jq -r .csrfToken)

# Post credentials:
curl -sS -L -c /tmp/fastcom-cookies.txt -b /tmp/fastcom-cookies.txt \
...
curl -sS -b /tmp/fastcom-cookies.txt \
  -o /dev/null -w "settings: HTTP %{http_code}\n" http://localhost:13007/api/settings

docker rm -f fastcom-auth-final
docker rmi fastcom-auth-final
```
