...
Run: `docker build -t fastcom-docker-fastcom:alerts-test .`
Expected: build succeeds. Size should be within ~5 MB of the previous 1.11 GB.

- [ ] **Step 3: Smoke-test the container**

```
docker run -d --name fastcom-alerts -p 13002:3000 fastcom-docker-fastcom:alerts-test
sleep 6
curl -sS http://localhost:13002/api/alerts/rules
docker rm -f fastcom-alerts
```
...
```
docker build -t fastcom-docker-fastcom:alerts .
docker run -d --name fastcom-alerts -p 13003:3000 \
  -e FASTCOM_NTFY_URL=https://ntfy.sh/fastcom-dev-$(date +%s) \
  fastcom-docker-fastcom:alerts
sleep 6
...
# Check ntfy.sh/fastcom-dev-<timestamp> in a browser for the notification.
curl -sS 'http://localhost:13003/api/alerts?limit=10'
docker rm -f fastcom-alerts
```

Expected: ntfy topic receives a "Fastcom: Download dropped below..." notification; `/api/alerts` returns the fired row with `delivery_status.ntfy.ok: true`.
